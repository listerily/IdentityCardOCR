...
def generate_card_number(code, gender, birth_year, birth_month, birth_day):
    ...
    identity_code_str += str(gender + random.randint(0, 4) * 2)
...
class Generator:
    ...
    def generate(self, write=False, show=True, add_noise=True, directory='./cards'):
        # Generate
        for i in range(self.count):
            self.generate_one_image(i, write, show, add_noise, directory)

    def generate_one_image(self, idx, write=False, show=True, add_noise=True, directory='./cards'):
...
if __name__ == '__main__':
    Generator(1000).generate(show=False, write=True)
